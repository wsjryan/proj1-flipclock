/**
 * autopush.js
 * 감시 파일 저장 시 자동으로 git commit + push
 * 실행: node autopush.js
 * 종료: Ctrl+C
 */
...
const FILES = ['index.html', 'autopush.js'];
...
function autoPush() {
  try {
    ...
    execSync(`git add ${FILES.join(' ')}`,    { cwd: __dirname, stdio: 'inherit' });
...
// 파일 감시 시작
FILES.forEach(file => {
  fs.watch(path.join(__dirname, file), (event) => {
